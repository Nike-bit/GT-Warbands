...
export function getDsMonsterDefensesBaseMagical(item) {
  if (!dsMonsterDefensesActive()) return undefined;
  return Boolean(item.getFlag(DS_MODULE_ID, "attackTraits")?.magical);
}

export function attachDsMonsterDefensesMagicalOverride(config, magical) {
  if (!dsMonsterDefensesActive() || typeof magical !== "boolean" || !config) return false;

  // Roll-time interoperability contract: provide the effective Skirmish trait
  // without altering DS-Monster-Defenses' persistent NPC Attack Item flag.
  config.gtWarbands ??= {};
...
export function activateDsMonsterDefensesSheetListeners({ sheet, item, root }) {
  ...
  for (const magical of root.querySelectorAll('[data-gtw-ds-field="magical"]')) {
